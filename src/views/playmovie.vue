...
Toast("请先登录！");
        ...
        if (this.$refs.msg.value == "") {
          ...
          let { data } = await myaxios(
            "POST",
            "http://47.103.222.56:8899/api/postcomment/" + this.id,
            obj
          );
          if (data.status == 0) {
            pushobj.msg = obj.msg;
  ...
  async created() {
    let { data } = await myaxios("GET", "http://47.103.222.56:8899/api/playIdMovie/" + this.id);
    if (data.status == 0) {
      this.flag = true;
    ...
    let { data: idata } = await myaxios("GET", "http://47.103.222.56:8899/api/getcontent/" + this.id);
    if (idata.status == 0) {
      this.content = idata.message;
    }
    let { data: adata } = await myaxios("GET", "http://47.103.222.56:8899/api/getintroduce/" + this.id);
    if (adata.status == 0) {
    ...
    let { data: com } = await myaxios("GET", "http://47.103.222.56:8899/api/getcomments/" + this.id);
    // console.log(com)
    if (com.status == 0) {
