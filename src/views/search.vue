<template>
  <div class="contain">
        ...
        <div class="mui-scroll" >
          <div class="movielist">
            <div class="movieactive" @touchstart='touchstart($event)' @touchmove='touchmove($event)' @touchend='touchend($event)' v-for="item in showlist" :key="item.v_id" @click='play(item.v_id)'>
              <img v-lazy="item.v_pic" />
              <div class="moviename">{{item.v_name}}</div>
              <div class="moviei">{{item.v_publishyear}}/{{item.v_publisharea}}/{{item.type}}</div>
            </div>
          </div>
         <div class='footer'>
            已经到底啦ヽ(≧□≦)ノ
...
export default {
  data: function() {
   ...
   methods: {
     touchstart(e){
       if(e.target.parentNode.className=='movieactive'||e.target.parentNode.className=='movieintroduce'){
   ...
   async play(id){
      ...
      let {data}=await myaxios("GET",'http://47.103.222.56:8899/api/updatehit/'+id)       
  ...
  async created() {
    let { data } = await myaxios("GET", "http://47.103.222.56:8899/api/getallmovie");   
    if (data.status == 0) {
    ...
    let { data: idata } = await myaxios("GET", "http://47.103.222.56:8899/api/getonelist"); 
    if (idata.status == 0) {
...
.movieactive {
  ...
  text-overflow: ellipsis !important;
  white-space: nowrap !important;
  box-shadow: 4px 2px 2px rgb(192, 191, 191);
  width: 30%;
  height: 180px;
  float: left;
  margin-left: 10px;
...
// .typelist .mui-slider:nth-child(1) {
//   // margin-top: 15px;
// }
.mui-slider .type {
  margin-left: 30px;
